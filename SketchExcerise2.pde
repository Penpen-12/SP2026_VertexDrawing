/**
  ...
  Project Description: This program is for creating complex curvy shapes and animarion using codes. 
  */
void setup() {
  size (600, 400);
  frameRate(30);
...
//background hatch
  ...
  int f= int (frameCount%frameRate); //castting
  println(f); 
  if (f>frameRate/2){
    f=1*(int (frameRate)-f);
  }
...
void mouseDragged () { //create a brush
  stroke (64, 64, 64, 255);
  strokeWeight (1);
